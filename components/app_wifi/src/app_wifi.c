...
#define IO_CONNECT_PHONE_APP_G_LINK   "https://play.google.com/store/apps/details?id=com.codor.stelian.ioconnect"
#define IO_CONNECT_PHONE_APP_HW_LINK   "https://appgallery.cloud.huawei.com/ag/n/app/C104694841"
#define IO_CONNECT_PHONE_APP_APK_LINK   "https://drive.google.com/file/d/1j-78cbTYLboyCpS3v1mciMzks87leLDc/view?usp=sharing"
char esp_rainmaker_ascii_art[] = \
"   ___ ___     ____                            _   \n"\
"  |_ _/ _ \\   / ___|___  _ __  _ __   ___  ___| |_ \n"\
"   | | | | | | |   / _ \\| '_ \\| '_ \\ / _ \\/ __| __|\n"\
"   | | |_| | | |__| (_) | | | | | | |  __/ (__| |_ \n"\
"  |___\\___/   \\____\\___/|_| |_|_| |_|\\___|\\___|\\__|\n";

static void intro_print(bool provisioned)
{
    printf("####################################################################################################\n");
    printf("%s\n", esp_rainmaker_ascii_art);
    printf("Welcome to IO Connect %s demo application!\n", RMAKER_DEMO_PROJECT_NAME);
    if (!provisioned) {
        printf("Follow these steps to get started:\n");
        printf("1. Download the IO Connect Android app from Google Play Store:\n");
        printf("   %s\n", IO_CONNECT_PHONE_APP_G_LINK);
        printf("   Download the IO Connect Android app from Huawei AppGallery:\n");
        printf("   %s\n", IO_CONNECT_PHONE_APP_HW_LINK);
        printf("   Download the IO Connect Android app from Google Drive:\n");
        printf("   %s\n", IO_CONNECT_PHONE_APP_APK_LINK);
...
static void app_wifi_print_qr(const char *name, const char *pop, const char *transport)
{
    ...
    ESP_LOGI(TAG, "Scan this QR code from the IO Connect phone app for Provisioning.");
...
esp_err_t app_wifi_init(const struct app_wifi_config *config)
{
	...
	wifi_connect = config->wifi_connect ? config->wifi_connect : esp_wifi_connect;
...
esp_err_t get_dev_mac(char *mac)
{
    ...
    esp_err_t err = esp_wifi_get_mac(WIFI_IF_STA, eth_mac);
    if (err == ESP_OK) {
        snprintf(mac, MAC_STR_SIZE, "%02x:%02x:%02x:%02x:%02x:%02x", eth_mac[0], eth_mac[1], eth_mac[2], eth_mac[3], eth_mac[4], eth_mac[5]);
        return ESP_OK;
    } else {
...
{
	snprintf(pop, POP_STR_SIZE, "%s", get_device_pop(pop_type));
	return ESP_OK;
}